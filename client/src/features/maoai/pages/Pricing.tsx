...
export default function MaoAIPricing() {
  ...
  const handleSelectPlan = (tier: PlanTier) => {
    if (!user) { window.location.href = MAOAI_ROUTES.LOGIN; return; }
  ...
  const handleCreateOrder = async (provider: string) => {
    ...
    try {
      const result = await createOrderMutation.mutateAsync({
        plan: "content", // map tier to plan enum
    ...
    <div className="min-h-screen bg-[#080808] text-white" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <header className="border-b border-white/8 bg-[#080808]/95 backdrop-blur-sm sticky top-0 z-20">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          <a href={MAOAI_ROUTES.CHAT} className="flex items-center gap-2 text-[#C9A84C] hover:opacity-80 transition-opacity">
            <Cat size={18} />
          ...
          <div className="flex items-center gap-3">
            ...
            {user ? (
              <a href={MAOAI_ROUTES.CHAT} className="flex items-center gap-1.5 px-3 py-1.5 bg-[#C9A84C]/10 border border-[#C9A84C]/30 text-[#C9A84C] text-xs hover:bg-[#C9A84C]/20 transition-all">
                {t("返回对话", "Back to Chat", currency)} <ArrowRight size={11} />
              </a>
            ) : (
              <a href={MAOAI_ROUTES.LOGIN} className="flex items-center gap-1.5 px-3 py-1.5 bg-[#C9A84C]/10 border border-[#C9A84C]/30 text-[#C9A84C] text-xs hover:bg-[#C9A84C]/20 transition-all">
